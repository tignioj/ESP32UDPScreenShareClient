...
2 个“内置”预设由程序提供，只能应用，不能修改或删除。调整地址或色彩模式后，点击“保存为个人预设”即可保存服务器地址、UDP 端口和模式；可以保存多份，也可以覆盖或删除已选中的个人预设。
...
| --- | --- | --- | --- | --- |
| 240 高帧率 RGB332 | 240 × 240 | 8 bit | 6 行 / 40 包 | 30 Mbit/s；47.0 FPS 上限；两帧在途 |
| 240 高画质 RGB565 | 240 × 240 | 16 bit 大端 | 3 行 / 80 包 | 30 Mbit/s；25.5 FPS 上限；两帧在途 |
...
采集线程遵守当前图像源声明的帧率，只编码可能被使用的新画面；当图像源帧率低于 UDP 显示帧率时，发送线程复用最新完整载荷，因此不会为重复画面反复运行音频特效、屏幕捕获和色彩编码。推流预览解码限制为 10 FPS。当前音频可视化场景实测由约 5.28 个逻辑核心降至 2.08 个，完整显示仍约为 46.5 FPS；具体 CPU 占用会随启用的效果、屏幕捕获范围和处理器变化。
