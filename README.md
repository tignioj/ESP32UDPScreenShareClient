...
- 音频可视化可不经 UDP 推流，以可调帧率的透明桌面悬浮窗独立运行
...
选择音频可视化源后，可在工作台的“独立透明窗口”区域直接启动画面，不需要点击“开始推流”。“窗口 FPS”允许范围为 1–120，应用后同时保存为该音频源的生成帧率；效果组合预设仍在原来的“效果组合预设”区域选择和管理，并会实时作用于独立窗口。

独立窗口默认置顶且无边框：按住画面拖动可移动，使用鼠标滚轮可缩放，右键或按 `Esc` 可关闭。Windows 使用透明背景；其他不支持透明色窗口的 Tk 平台会自动降级为半透明无边框窗口。
